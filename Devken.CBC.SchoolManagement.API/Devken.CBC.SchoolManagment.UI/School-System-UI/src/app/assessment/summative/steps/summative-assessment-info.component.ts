// ═══════════════════════════════════════════════════════════════════
// steps/summative-assessment-info.component.ts
// ═══════════════════════════════════════════════════════════════════
...
@Component({
  selector: 'app-summative-assessment-info',
  standalone: true,
...
<div class="max-w-3xl mx-auto">
  <div class="mb-8">
    ...
    <p class="text-gray-500 dark:text-gray-400 mt-1">Enter the core details for this summative assessment.</p>
  </div>
  ...
  <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 sm:p-8 space-y-6">
    ...
    <div>
      ...
      <textarea
        ...
        class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600
               bg-white dark:bg-gray-700 text-gray-900 dark:text-white
               focus:ring-2 focus:ring-violet-500 focus:border-violet-500 outline-none transition resize-none">
      </textarea>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div>
        ...
        <select
          [(ngModel)]="data.examType"
          (ngModelChange)="onChange()"
          class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600
                 bg-white dark:bg-gray-700 text-gray-900 dark:text-white
                 focus:ring-2 focus:ring-violet-500 outline-none transition"
          [class.border-red-400]="touched && !data.examType">
          <option value="">Select exam type</option>
          <option *ngFor="let t of examTypes" [value]="t">{{ t }}</option>
        </select>
        <p *ngIf="touched && !data.examType" class="text-red-500 text-xs mt-1">Exam type is required</p>
      </div>
...
export class SummativeAssessmentInfoComponent implements OnInit, OnChanges {
  @Input() formData:      any = {};
  @Input() classes:       any[] = [];
  @Input() teachers:      any[] = [];
  @Input() subjects:      any[] = [];
  ...
  @Input() examTypes:     readonly string[] = [];
  ...
  private emitValid(): void {
    this.formValid.emit(!!(this.data.title?.trim() && this.data.examType && this.data.assessmentDate && this.data.maximumScore));
  }
}