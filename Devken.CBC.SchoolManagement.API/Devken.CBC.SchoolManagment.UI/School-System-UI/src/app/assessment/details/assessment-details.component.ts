...
export class AssessmentDetailsComponent implements OnInit, OnDestroy {
  ...
  // AssessmentDto is the full response shape (includes all type-specific fields)
  ...
  ngOnInit(): void {
    ...
    const type      = typeParam ? Number(typeParam) as AssessmentType : null;
  ...
  get coreDetailItems(): DetailItem[] {
    ...
    return [
      ...
      { label: 'Score Entries',   value: `${a.scoreCount ?? 0} student(s) scored`,                    icon: 'assignment_turned_in', type: 'text'   },
  ...
  get formativeDetailItems(): DetailItem[] {
    ...
    const items: DetailItem[] = [
      ...
      { label: 'Strand',            value: a.strandName,                                          icon: 'account_tree', type: 'text'    },
      { label: 'Sub-Strand',        value: a.subStrandName,                                       icon: 'device_hub',   type: 'text'    },
      { label: 'Assessment Weight', value: a.assessmentWeight ? `${a.assessmentWeight}%` : null,  icon: 'percent',      type: 'text'    },
      { label: 'Requires Rubric',   value: a.requiresRubric != null ? (a.requiresRubric ? 'Yes' : 'No') : null, icon: 'table_chart', type: 'boolean' },
    ...
    return items.filter(i => i.value != null && i.value !== '' && i.value !== '—');
  }
  ...
  get summativeDetailItems(): DetailItem[] {
    ...
    const items: DetailItem[] = [
      ...
      { label: 'Pass Mark',        value: a.passMark != null ? `${a.passMark}%` : null,                    icon: 'percent',              type: 'text'    },
      { label: 'Has Practical',    value: a.hasPracticalComponent != null ? (a.hasPracticalComponent ? 'Yes' : 'No') : null, icon: 'science', type: 'boolean' },
      { label: 'Practical Weight', value: a.hasPracticalComponent ? `${a.practicalWeight}%` : null,       icon: 'percent',              type: 'text'    },
      { label: 'Theory Weight',    value: a.hasPracticalComponent ? `${a.theoryWeight}%` : null,          icon: 'percent',              type: 'text'    },
  ...
  get competencyDetailItems(): DetailItem[] {
    ...
    const items: DetailItem[] = [
      ...
      { label: 'Assessment Method', value: a.assessmentMethod ? this.getMethodName(a.assessmentMethod) : null,      icon: 'visibility',    type: 'text'    },
      { label: 'Rating Scale',      value: a.ratingScale,                                                           icon: 'stars',         type: 'text'    },
      { label: 'Observation Based', value: a.isObservationBased != null ? (a.isObservationBased ? 'Yes' : 'No') : null, icon: 'visibility', type: 'boolean' },
