...
// Bulk payment entry — shared settings + manual rows + class import
// ═══════════════════════════════════════════════════════════════════
    ...
    <div class="flex items-center gap-3 min-w-0">
      ...
      <div class="flex items-center gap-3 min-w-0">
        <div class="flex items-center justify-center w-9 h-9 rounded-xl shadow-md flex-shrink-0
          ...
          <mat-icon class="text-white text-base">playlist_add</mat-icon>
        </div>
        <div class="min-w-0">
  ...
  <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <mat-card-content class="!px-5 !pt-5 !pb-4 space-y-4">
        ...
        <div class="max-h-64 overflow-y-auto rounded-xl border
          ...
          <div *ngFor="let s of filteredClassStudents; trackBy: trackById"
            ...
            [class.dark:bg-indigo-45]="s.selected"
        ...
        <div class="flex items-center justify-between pt-2 border-t border-gray-200 dark:border-gray-700">
          ...
          <button mat-flat-button
            [disabled]="newSelectionCount === 0 || isImporting"
    ...
    <div class="space-y-3">
      ...
      <div *ngIf="rows.length === 0"
        class="flex flex-col items-center gap-3 py-14 border-2 border-dashed
               border-gray-300 dark:border-gray-700 rounded-2xl">
        <mat-icon class="text-gray-300 dark:text-gray-600 !w-14 !h-14 !text-5xl">playlist_add</mat-icon>
        <p class="text-sm font-semibold text-gray-500">No payment rows yet</p>
        <div class="flex gap-3 mt-1">
          <button mat-stroked-button (click)="toggleClassPanel()"
            class="border-indigo-200 text-indigo-700 hover:bg-indigo-50">
            <mat-icon>groups</mat-icon><span class="ml-1">Import by Class</span>
          </button>
          <button mat-flat-button (click)="addRow()" class="!bg-emerald-600 !text-white">
      ...
      <mat-card *ngFor="let row of rows; let i = index; trackBy: trackByIndex"
        ...
        <div class="flex items-center justify-between px-4 py-2.5
          ...
          <div class="flex items-center gap-2 flex-shrink-0">
            ...
            <button mat-icon-button class="!w-7 !h-7" (click)="removeRow(i)"
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <mat-form-field appearance="outline" class="w-full">
              ...
              <mat-select [(ngModel)]="row.studentId"
                ...
                <mat-option disabled class="!h-auto !p-0">
                  <div class="px-3 py-2 sticky top-0 bg-white dark:bg-gray-800 z-10
                    ...
                    <input
                      class="w-full border border-gray-300 dark:border-gray-600 rounded-lg
                             px-3 py-1.5 text-sm outline-none focus:ring-2 focus:ring-teal-400
                             bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                      placeholder="Type to search…"
                      (keydown.Space)="$event.stopPropagation()"
                      [(ngModel)]="row._studentSearch"
                      (ngModelChange)="filterRowStudents(row)" />
                  </div>
                </mat-option>
                <mat-option value="">— Select student —</mat-option>
                <mat-option *ngFor="let s of row._filteredStudents" [value]="s.id">
                  {{ s.firstName }} {{ s.lastName }}
...
export class PaymentBulkFormComponent implements OnInit, OnDestroy {
  ...
  isRowIncomplete(row: BulkRow): boolean {
    return !!row.studentId && !this.isRowComplete(row);
  }
  ...
  // Only count students that are selected AND not already added
  ...
  importSelectedStudents(): void {
    // Only students selected AND not already in rows
    ...
    .subscribe((invoicesPerStudent: any[]) => {
      toImport.forEach((student, idx) => {
        ...
        const autoInvoice = invoices.length === 1 ? invoices[0].id : '';
  ...
  onRowStudentChange(row: BulkRow, studentId: string): void {
    ...
    this._service
      ...
      .subscribe({
        next: inv => {
          row._invoices = inv;
          if (inv.length === 1) row.invoiceId = inv[0].id;
        },
        error: () => { row._error = 'Failed to load invoices'; },
      });
  }

  filterRowStudents(row: BulkRow): void {
    const q = row._studentSearch.toLowerCase();
    row._filteredStudents = q
      ? this.importStudents.filter(s =>
          `${s.firstName} ${s.lastName}`.toLowerCase().includes(q) ||
          (s.admissionNo || s.admissionNumber || '').toLowerCase().includes(q))
      : [...this.importStudents];
  }
  ...
  getMethodIcon(m: string): string {
    return ({ Cash:'payments', Mpesa:'phone_android', BankTransfer:'account_balance',
              Cheque:'description', Card:'credit_card', Online:'language' } as any)[m] ?? 'payments';
  }

  getMethodLabel(m: string): string {
    return ({ Cash:'Cash', Mpesa:'M-Pesa', BankTransfer:'Bank Transfer',
              Cheque:'Cheque', Card:'Card', Online:'Online' } as any)[m] ?? m;
  }
