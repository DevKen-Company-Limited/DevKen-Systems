...
export class InvoiceService {
  ...
  getByStudent(studentId: string, schoolId?: string): Observable<ApiResponse<InvoiceSummaryResponseDto[]>> {
    let params = new HttpParams();
    if (schoolId) params = params.set('schoolId', schoolId);
    return this.http.get<ApiResponse<InvoiceSummaryResponseDto[]>>(
      `${this.baseUrl}/by-student/${studentId}`, { params });
  }

  /** GET /api/finance/invoices/by-parent/{parentId} */
  getByParent(parentId: string): Observable<ApiResponse<InvoiceSummaryResponseDto[]>> {
    return this.http.get<ApiResponse<InvoiceSummaryResponseDto[]>>(
      `${this.baseUrl}/by-parent/${parentId}`);
  }
  ...
  applyDiscount(id: string, dto: ApplyDiscountDto): Observable<ApiResponse<InvoiceResponseDto>> {
    return this.http.patch<ApiResponse<InvoiceResponseDto>>(
      `${this.baseUrl}/${id}/apply-discount`, dto);
  }
   ...
   *
   * Asks the backend to recompute StatusInvoice from the sum of all
   * Completed non-reversal payments against this invoice.
