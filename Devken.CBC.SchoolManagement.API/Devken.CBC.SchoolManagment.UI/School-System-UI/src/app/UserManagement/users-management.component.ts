// app/modules/admin/users-management/users-management.component.ts
import {
    Component,
    OnInit,
    OnDestroy,
...
export class UsersManagementComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    loadAll(): void {
        ...
        this._service.getAll()
            ...
            .subscribe({
                ...
                error: (err) => {
                    this.isLoading = false;
                    this._alert.error(err?.error?.message || 'Failed to load users');
                },
            });
    }
     ...
     *
     * Calls UserService.adminResetPassword(email) which hits
     * POST /api/auth/forgot-password — the backend sends a secure reset link
     * directly to the user's inbox. The admin never handles credentials.
     *
     * The backend always returns 200 OK (enumeration guard), so we show a
     * generic success message regardless of the payload.
     */
    resetPassword(user: UserDto): void {
        // Guard: a user without an email address cannot receive a reset link
        if (!user.email) {
            this._alert.error('This user has no email address on record.');
            return;
        }

        this._alert.confirm({
            title      : 'Reset Password',
            message    : `Send a password-reset email to ${user.firstName} ${user.lastName} (${user.email})?
