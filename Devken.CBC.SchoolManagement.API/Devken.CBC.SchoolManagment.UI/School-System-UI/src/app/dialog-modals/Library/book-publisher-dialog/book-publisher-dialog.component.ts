...
export interface BookPublisherDialogData {
  mode: 'create' | 'edit';
  item?: BookPublisherResponseDto;
}

@Component({
  selector: 'app-book-publisher-dialog',
  standalone: true,
  imports: [
  ...
  template: `
    <!-- Header -->
    <div class="bg-gradient-to-r from-orange-500 via-amber-500 to-yellow-500 relative overflow-hidden rounded-t-xl px-6 py-5">
      <div class="absolute inset-0 opacity-10 pointer-events-none">
        <div class="absolute top-0 right-0 w-32 h-32 bg-orange-200 rounded-full blur-2xl"></div>
        <div class="absolute bottom-0 left-0 w-40 h-40 bg-yellow-300 rounded-full blur-2xl"></div>
      </div>
      <div class="relative z-10 flex items-center gap-3 pr-10">
        <div class="w-10 h-10 bg-white/20 backdrop-blur-sm rounded-xl flex items-center justify-center shrink-0">
          <mat-icon class="text-white icon-size-6">business</mat-icon>
        </div>
        <div>
          <h2 class="text-xl font-bold text-white m-0 leading-tight">{{ isEditMode ? 'Edit Publisher' : 'Add New Publisher' }}</h2>
          <p class="text-white/80 text-sm mt-0.5 mb-0">
            {{ isEditMode ? 'Updating: ' + data.item?.name : 'Fill in the details to create a new publisher' }}
          </p>
        </div>
      </div>
    ...
    <div *ngIf="isSaving" class="absolute inset-0 bg-white/95 dark:bg-gray-900/95 z-50 flex items-center justify-center backdrop-blur-sm">
      <div class="text-center p-8">
        <div class="relative w-16 h-16 mx-auto mb-4">
          ...
          <mat-icon class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-amber-600">business</mat-icon>
        </div>
    ...
    <form [formGroup]="form" (ngSubmit)="onSubmit()" mat-dialog-content class="p-5 space-y-4">
      ...
      <mat-form-field appearance="outline" class="w-full">
        <mat-label>Publisher Name <span class="text-red-500">*</span></mat-label>
        <input matInput formControlName="name" placeholder="e.g. Oxford University Press" autocomplete="off">
      ...
      <mat-form-field appearance="outline" class="w-full">
        ...
        <textarea matInput formControlName="address" placeholder="Optional address..." rows="3" maxlength="500"></textarea>
        <mat-icon matPrefix>location_on</mat-icon>
        <mat-hint align="end">{{ addressLength }}/500</mat-hint>
...
export class BookPublisherDialogComponent implements OnInit, OnDestroy {
  ...
  get addressLength(): number { return this.form?.get('address')?.value?.length ?? 0; }

  constructor(
    private readonly _fb:            FormBuilder,
    private readonly _service:       BookPublisherService,
    private readonly _dialogRef:     MatDialogRef<BookPublisherDialogComponent>,
    @Inject(MAT_DIALOG_DATA) public  data: BookPublisherDialogData,
