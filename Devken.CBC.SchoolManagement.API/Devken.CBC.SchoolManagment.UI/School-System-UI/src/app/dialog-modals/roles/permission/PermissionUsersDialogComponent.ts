...
import { Permission, UserWithPermission } from 'app/core/DevKenService/Types/role-permissions';
import { Subject, debounceTime, distinctUntilChanged, takeUntil } from 'rxjs';

export interface PermissionUsersDialogData {
...
@Component({
    selector: 'app-permission-users-dialog',
    standalone: true,
    imports: [
        CommonModule,
        FormsModule,
        ...
        <div class="flex flex-col max-h-[85vh]">
            ...
            <div class="flex items-start gap-4 p-6 pb-4">
                ...
                <div class="flex-1 min-w-0">
                    <h2 mat-dialog-title class="text-2xl font-bold text-gray-900 dark:text-white mb-1">
                        {{ data.title || (data.permission ? 'Users with Permission' : 'Users in Role') }}
                    </h2>
                    <div class="text-sm text-gray-600 dark:text-gray-400" *ngIf="data.permission">
                        <strong>{{ data.permission.displayName }}</strong>
                    </div>
                    <div class="text-sm text-gray-600 dark:text-gray-400" *ngIf="data.role">
                        <strong>{{ data.role.roleName }}</strong> Role
                    </div>
                    <div class="text-xs font-mono text-gray-500 dark:text-gray-500 mt-1 bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded inline-block" 
                         *ngIf="data.permission">
                        {{ data.permission.key }}
                    </div>
                </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2 text-blue-900 dark:text-blue-100">
                            ...
                            <span class="font-semibold">
                                ...
                                {{ data.permission ? 'have this permission' : 'are in this role' }}
                            </span>
                        </div>
            ...
            <div mat-dialog-content class="flex-1 overflow-y-auto p-6">
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex flex-wrap gap-1.5 items-center">
                                    <span class="text-xs text-gray-500 dark:text-gray-400 flex-shrink-0">Roles:</span>
                                    <mat-chip-row 
                                        *ngFor="let roleName of user.roleNames; trackBy: trackByRoleName"
                                        class="text-xs h-6">
                ...
                <div *ngIf="data.users.length === 0" class="text-center py-12">
                    ...
                    <p class="text-gray-500 dark:text-gray-400" *ngIf="data.permission">
                        No users currently have this permission
                    </p>
                    <p class="text-gray-500 dark:text-gray-400" *ngIf="data.role">
...
export class PermissionUsersDialogComponent implements OnInit, OnDestroy {
    ...
    constructor(
        @Inject(MAT_DIALOG_DATA) public data: PermissionUsersDialogData,
        private dialogRef: MatDialogRef<PermissionUsersDialogComponent>
    ) {}
    ...
    private applyFilter(): void {
        if (!this.searchTerm || !this.searchTerm.trim()) {
            ...
            this.filteredUsers = this.allUsers.filter(user =>
                ...
                user.roleNames?.some(role => role.toLowerCase().includes(term))
            );
        }
