...
namespace Devken.CBC.SchoolManagement.Infrastructure.Services
{
    ...
    public class PermissionSeedService : IPermissionSeedService
    {
        ...
        public async Task SeedAllSchoolsAsync()
        {
            ...
            // 1. Ensure global permissions exist once — this always runs so
            //    newly added PermissionCatalogue entries are inserted regardless
            //    of whether any school is "skipped" below.
            var permissionMap = await EnsurePermissionsAsync();
            await _context.SaveChangesAsync();
            ...
            _logger.LogInformation("Found {Count} school(s) to process", schoolIds.Count);
            ...
            {
                // Always call SeedRolesForSchoolAsync — it is fully idempotent:
                // - Creates missing roles
                // - Adds missing RolePermission rows for existing roles
                // - Never duplicates anything
                //
                // Previously this was skipped when a school "already had all roles",
                // which meant newly added permissions (e.g. InvoiceItem.Read) were
                // never wired up to existing roles.
                ...
                bool hadAllRoles = allExpectedNames.All(n => existingRoleNames.Contains(n));
                ...
                {
                    _logger.LogDebug(
                        "School {SchoolId} already had all roles — patched any missing permissions",
                        schoolId);
                    patched++;
                }
                else
                {
                    _logger.LogInformation(
                        "Seeded missing roles for school {SchoolId}", schoolId);
                    seeded++;
                }
            }

            _logger.LogInformation(
                "Seed complete. Newly seeded: {Seeded}, Patched existing: {Patched}",
                seeded, patched);
        }
        ...
        private async Task<Dictionary<string, Guid>> EnsurePermissionsAsync()
        {
            ...
            if (toAdd.Count > 0)
            {
                ...
                _logger.LogInformation("Added {Count} new permission(s)", toAdd.Count);
            }
        ...
        ///
        /// For roles that already exist, any missing RolePermission rows are added
        /// so newly introduced permission keys are always picked up.
        ///
            ...
            Dictionary<string, Guid> permissionMap)
        {
            ...
            foreach (var (roleName, description, isSystem, permissions) in DefaultRoles.All)
            {
                ...
                {
                    // Role exists — patch any missing RolePermission rows.
                    // This is the critical path for existing schools when new
                    // permission keys are added to the catalogue.
                    role = existingRole;
